...
def perlin(size, cells=1, batch=None):
    ...
    ndim = size.numel()
    cells = torch.as_tensor(cells).ravel().expand(ndim)
    ...
    batch = torch.as_tensor([] if batch is None else batch).ravel()
    grad = torch.rand(ndim, *batch, *cells).mul(2).sub(1)
    grad = grad.view(ndim, *batch, -1)

    # Output grid, subsampling between gradient coordinates.
    grid = (torch.linspace(0, c - 1, s) for c, s in zip(cells, size))
...
def octaves(size, freq, pers, batch=None):
    ...
    freq : sequence of int or torch.Tensor
        Perlin-noise frequency levels to add. Wiss use `2 ** freq` cells, which
        must be less than `size`, in each dimension.
    ...
    freq = torch.as_tensor(freq).ravel()
    pers = torch.as_tensor(pers)
    ...
    out = 0
    for f in freq:
        out += perlin(size, cells=2 ** f, batch=batch) * pers ** f
